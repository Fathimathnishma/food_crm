...
class _OrderSummeryScreenState extends State<OrderSummeryScreen>
  ...
  void initState() {
    ...
    // Initialize TabController with default length 1
    tabController = TabController(length: 1, vsync: this);
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<OrderSummeryProvider>(
      builder: (context, stateAddOrder, child) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          final newTabCount = stateAddOrder.itemsList.length + 1; // Add "Total" tab
          if (newTabCount != tabController.length) {
            _updateTabController(newTabCount);
          }
        });
